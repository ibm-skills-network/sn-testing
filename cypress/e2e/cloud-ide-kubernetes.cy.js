...
describe("Cloud IDE Kubernetes", () => {
  ...
  it("can run simple docker commands", () => {
    const commands = ["docker ps", "docker run hello-world"];
  ...
  it("can run docker and have connectivity", () => {
    ...
    checkTerminalOutput(commands, { wait: 15000 }).should(
      "include",
  ...
  it("can run simple kubernetes", () => {
    ...
    cy.theia().terminal(
      "wget https://raw.githubusercontent.com/mngaonkar/single-node-kubernetes-cluster/master/nginx-deployment.yaml{enter}",
    );
    cy.theia().terminal("kubectl apply -f nginx-deployment.yaml{enter}");
    cy.theia().terminal(
      "kubectl port-forward $(kubectl get pods --output=jsonpath={.items[]..metadata.name}) 8088:80",
      false,
    );
    ...
    cy.get("@proxyURL").then((proxyURL) => {
      ...
      cy.theia().terminal("{ctrl}c");
      cy.theia().terminal(
        "kubectl deconste deployments nginx-deployment{enter}",
      );
    });
  });
});
